...
from t5_utils import initialize_model, initialize_optimizer_and_scheduler, save_model, load_model_from_checkpoint, setup_wandb, mkdir
from transformers import GenerationConfig, T5TokenizerFast as T5Tokenizer
...
def get_args():
    ...
    parser.add_argument('--beam_size', type=int, default=4, help="Beam size to use during generation for evaluation")
    parser.add_argument('--max_new_tokens', type=int, default=256, help="Max new tokens to generate during evaluation")
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, decoder_input, decoder_targets, initial_decoder_inputs in tqdm(dev_loader):
            ...
            generations+= [q.strip() for q in tokenizer.batch_decode(generated_ids, skip_special_tokens=True)]
            if not printed:
                printed = True
                print("===============================")
                print(f"input: {tokenizer.batch_decode(encoder_input, skip_special_tokens=True)}")
                print(f"Generated: {tokenizer.batch_decode(generated_ids, skip_special_tokens=True)}")
    save_queries_and_records(generations, model_sql_path, model_record_path)

    sql_em, record_em, record_f1, error_msgs = compute_metrics(gt_sql_pth, model_sql_path, gt_record_path, model_record_path)
    error_rate = sum(1 for m in error_msgs if m != "") / len(error_msgs)
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, initial_decoder_inputs in tqdm(test_loader):
            ...
            generations += [q for q in tokenizer.batch_decode(generated_ids, skip_special_tokens=True)]
    save_queries_and_records(generations, model_sql_path, model_record_path)
